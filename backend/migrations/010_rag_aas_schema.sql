...
-- RAG Documents 인덱스
CREATE INDEX IF NOT EXISTS idx_rag_docs_tenant_active ON rag.rag_documents (tenant_id, is_active) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_rag_docs_source ON rag.rag_documents (source_type, source_id);
CREATE INDEX IF NOT EXISTS idx_rag_docs_tags ON rag.rag_documents USING GIN (tags);
CREATE INDEX IF NOT EXISTS idx_rag_docs_metadata ON rag.rag_documents USING GIN (metadata);
CREATE INDEX IF NOT EXISTS idx_rag_docs_text_search ON rag.rag_documents USING GIN (to_tsvector('korean', text));
...
-- =====================================================
-- RAG Document Versions (문서 버전 이력)
-- =====================================================
CREATE TABLE IF NOT EXISTS rag.rag_document_versions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_rag_doc_versions_document ON rag.rag_document_versions (document_id, version DESC);

COMMENT ON TABLE rag.rag_document_versions IS 'RAG 문서 버전 이력';

-- =====================================================
-- RAG Embeddings (임베딩 벡터 - pgvector)
-- =====================================================
CREATE TABLE IF NOT EXISTS rag.rag_embeddings (
...
CREATE INDEX IF NOT EXISTS idx_rag_embeddings_vector ON rag.rag_embeddings
...
-- =====================================================
-- Embedding Jobs (임베딩 생성 작업)
-- =====================================================
CREATE TABLE IF NOT EXISTS rag.embedding_jobs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL REFERENCES core.tenants(tenant_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_embedding_jobs_tenant ON rag.embedding_jobs (tenant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_embedding_jobs_status ON rag.embedding_jobs (status) WHERE status IN ('pending', 'running');

COMMENT ON TABLE rag.embedding_jobs IS '임베딩 생성 배치 작업';

-- =====================================================
-- Memories (AI 에이전트 장단기 메모리)
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_memories_tenant_type ON rag.memories (tenant_id, type);
CREATE INDEX IF NOT EXISTS idx_memories_expires ON rag.memories (expires_at) WHERE expires_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_memories_importance ON rag.memories (importance DESC);
...
-- AAS Assets (자산 정의)
-- =====================================================
CREATE TABLE IF NOT EXISTS aas.aas_assets (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL REFERENCES core.tenants(tenant_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_aas_assets_tenant ON aas.aas_assets (tenant_id);
CREATE INDEX IF NOT EXISTS idx_aas_assets_ref ON aas.aas_assets (ref_code);
CREATE INDEX IF NOT EXISTS idx_aas_assets_type ON aas.aas_assets (asset_type);
...
-- AAS Submodels (서브모델)
-- =====================================================
CREATE TABLE IF NOT EXISTS aas.aas_submodels (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_aas_submodels_asset ON aas.aas_submodels (asset_id);
CREATE INDEX IF NOT EXISTS idx_aas_submodels_category ON aas.aas_submodels (category);
...
-- AAS Elements (요소)
-- =====================================================
CREATE TABLE IF NOT EXISTS aas.aas_elements (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_aas_elements_submodel ON aas.aas_elements (submodel_id);

COMMENT ON TABLE aas.aas_elements IS 'AAS 요소 (데이터 포인트)';

-- =====================================================
-- AAS Source Mappings (소스 매핑)
-- =====================================================
CREATE TABLE IF NOT EXISTS aas.aas_source_mappings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL REFERENCES core.tenants(tenant_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_aas_mappings_element ON aas.aas_source_mappings (element_id);
CREATE INDEX IF NOT EXISTS idx_aas_mappings_tenant ON aas.aas_source_mappings (tenant_id);
...
-- 샘플 RAG 문서 (불량 대응 작업 지침서)
-- INSERT INTO rag.rag_documents (tenant_id, source_type, source_id, title, section, chunk_index, chunk_total, text, text_hash, word_count, char_count, metadata, tags)
-- SELECT
--     t.tenant_id,
--     'manual',
--     'SOP-001',
...
--     encode(sha256('불량이 발생하면...'::bytea), 'hex'),
--     25,
--     80,
