...
- **UI-FR-010 (Chat-Centric Visualization)**: 사용자의 데이터 조회 요청 시, 별도 페이지 이동 없이 채팅창 내에 **인터랙티브 차트(Embedded Chart)**를 렌더링한다.
- **UI-FR-020 (Dashboard Customization)**: 사용자는 대시보드에서 **'편집 모드(Edit Mode)'**를 통해 위젯의 위치를 드래그앤드롭으로 변경하고, 크기를 조절할 수 있다.
- **UI-FR-030 (Widget Add Flow)**: 채팅창의 분석 결과(차트)를 버튼 클릭 한 번으로 대시보드 위젯으로 **고정(Pin)**할 수 있어야 한다.
- **UI-FR-040 (Persistent Layout)**: 사용자가 변경한 대시보드 레이아웃은 DB에 저장되어, 재접속 시에도 유지되어야 한다.
